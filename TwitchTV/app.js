...
Per FFC comments, as it was advised not to use Twitch's API as it changes often and instead use a hard coded version of results
...
function fetchAllStreamers()
{

  var list = document.getElementById("resultsList");
  list.innerHTML = "";

	for(var i = 0; i < twitchResult.length; i++)
	{

		var currentStream = twitchResult[i];
		if(currentStream.stream != null)
		{
			var name = currentStream.stream.display_name;
			var logo = currentStream.stream.logo;
			var link = currentStream.stream.url;
			var banner = currentStream.stream.profile_banner;
      var li = document.createElement("li");
      li.innerHTML = "<a href="+link + " class='btn btn-link' target='_blank'>" + name + " </a>";
      li.setAttribute("style", "background:url("+banner+")");
			list.appendChild(li);
		}
...
function fetchOnlineStreamers()
{
  ...
  for(var i = 0; i < twitchResult.length; i++)
  {
    ...
    var currentStream = twitchResult[i];
    if(currentStream.stream != null && currentStream.stream.stream_type == "live")
    {
      var name = currentStream.stream.display_name;
      var logo = currentStream.stream.logo;
      var link = currentStream.stream.url;
...
function fetchOfflineStreamers()
{
  ...
  for(var i = 0; i < twitchResult.length; i++)
  {
    ...
    var currentStream = twitchResult[i];
    if(currentStream.stream != null && currentStream.stream.stream_type == "playlist")
    {
      var name = currentStream.stream.display_name;
      var logo = currentStream.stream.logo;
      var link = currentStream.stream.url;
